...
export async function openImageViewer(
  ...
  try {
    ...
    // maximized so it covers the screen like a fullscreen overlay. Raw
    // plugin commands with an explicit label: `Window.getByLabel` is broken
    // here — `get_all_windows` returns plain label strings in this Tauri
    // version, so `getByLabel` never matches and returns null.
